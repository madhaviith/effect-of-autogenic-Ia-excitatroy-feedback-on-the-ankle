####################################################################################################################################################################################
#This code imports the output data(muscle lenghts, muscle actiavtions, joint angles, afferent feedback, voltage traces) from the simulation folders- sim_o, sim_c and plots the data
#
...
o_folname = cwd + "/sim_o/output/"
c_folname = cwd + "/sim_c/output/"
...
for i in range(0,9):
    ax2[i].plot(c_time,voltage_dict_c[list(voltage_dict_c.keys())[i]][:,0]),#label=voltage_dict_c.keys()[0])
    ax2[i].set_title(list(voltage_dict_c.keys())[i],fontsize = 'xx-small',fontweight='bold')
...
for i in range(0,9):
    ax3[i].plot(o_time,voltage_dict_o[list(voltage_dict_o.keys())[i]][:,0]),#label=voltage_dict_c.keys()[0])
    ax3[i].set_title(list(voltage_dict_o.keys())[i],fontsize = 'xx-small',fontweight='bold')
